...
var id=0;
class Car {
    constructor(number_id,type,day,hour,special_day,direction,Entrance_to_road,current_section,Exit_from_road) {
...
function make_private_car (percent) {
  private=new Car(id++,types[0],0,0,0,0,0,0,0);
  if(percent>45){
...
function make_Bus (percent) {
  Bus=new Car(id++,types[1],0,0,0,0,0,0,0);
  if(percent>40){
...
function make_Truck (percent) {
  truck=new Car(id++,types[2],0,0,0,0,0,0,0,0,0);
  if(percent>30){
      truck.hour=hours[4]
...
//  }
    ...
    //kafka- after!!!
    // Car1=new Car(0,types[2],days.Sunday,"5:00",0,1,1,1,1,6,6);
    // console.log(JSON.stringify(Car1))
    // const kafka = require('./kafkaProduce');
    // t=JSON.parse(JSON.stringify(Car1))
